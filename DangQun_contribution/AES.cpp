...
// 转换十六进制为字符显示,只处理16个字节
//确保string空间为16字节以上
void AES::Hex2Str(string & src, string & dst)
{

	//输出时在加密串尾各添加一字符
	//字符由加密串首字节高4位加'A'组成
	//尾字符由'Z'-首字节高4位加'A'组成
	unsigned char chEnd='A'+(src[0]>>4);
	dst.push_back( chEnd );
	...
	for(int i=0; i< ENC_LEN; i++ )
	{
		...
		ctp=src[i];
		ctp>>=4;
		dst.push_back( ctp>9 ? 'A'+ctp-0xa : '0'+ctp);
	...
	dst.push_back('Z'-chEnd+'A');
}


void AES::Str2Hex(string & src, string & dst)
{
