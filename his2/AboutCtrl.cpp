...
CAboutCtrl::CAboutCtrl()
{
	...
	m_yPos = 0; 
	m_nStartHeight=0;
	m_strSubject="医疗信息管理系统解决方案 2003-2005";
...
/*************************************************************************/	
	HFONT pOldFont;
	COLORREF oldColor;
	HFONT hFont = CreateFont(14, 0, 0, 0, 
		...
		(LPSTR)"Arial");
	oldColor = SetTextColor(m_hBackgroundDC, RGB(55,46,173));
	pOldFont = (HFONT)SelectObject(m_hBackgroundDC, hFont);
	
	CRect di(0,0,cx,cy);
	CSize sz;
	GetTextExtentPoint32(m_hBackgroundDC,m_strSubject,m_strSubject.GetLength(),&sz);
	
	di.top=di.bottom-sz.cy-2;
	di.left=di.right-sz.cx;
	
	
	SetBkMode(m_hBackgroundDC,TRANSPARENT);
	DrawText(m_hBackgroundDC, m_strSubject, m_strSubject.GetLength(), &di, DT_TOP|DT_LEFT|DT_NOPREFIX | DT_SINGLELINE);
...
/**************************************************************************/				
	...
	// set scroll counter
//	m_nCounter = rect.Height()-m_nStartHeight;
	m_nCounter= di.bottom-20;
	ReleaseDC(pDC);
}
...
void CAboutCtrl::AnimateLogo()
{
	...
	BitBlt(m_hMemDC, 160, m_nCounter--, m_cxData, yPos, m_hCreditsDC, 0, 0, SRCAND);
...
/********************************************************************/
/*																	*/
/* Function name : SetSubject										*/
/* Description   : Member function to set subject text.				*/
/*																	*/
/********************************************************************/


void CAboutCtrl::SetSubject(LPCTSTR lpszSubject)
{
	m_strSubject=lpszSubject;
}